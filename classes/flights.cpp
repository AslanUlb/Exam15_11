	...
	do {
		...
		if (x == 1) {
			...
			do {
				...
				auto it = town.getCity().find(tempTown);
				if (it != town.getCity().end()) {
					f.pointSrc = tempTown;
					flag = 1;
				}
				else {
					cout << " Town is default .... try again" << endl;
				}
			} while (flag);
			do {
				flag = 0;
...
void printTicketPerson() {
	...
	for (it; it != listPerson.getTicketS().end(); ++it) {
		if (it->first == tempFlight)
			cout << it->first << "\t" << it->second.pass.fio << " : " << it->second.numberTicket << endl;
		else
